...
class AdminController extends Controller
{
    ...
    public function storeCompetition(Request $request)
    {
        ...
        $competition = Competition::create([
            ...
            'show_guidelines' => $request->has('show_guidelines') ? $request->boolean('show_guidelines') : true,
    ...
    public function updateCompetition(Request $request, $id)
    {
        ...
        $competition->update([
            ...
            'guidelines_file' => $guidelinesPath,
            'show_guidelines' => $request->has('show_guidelines') ? $request->boolean('show_guidelines') : false,
            'whatsapp_group_url' => $validated['whatsapp_group_url'] ?? null,
            'order' => isset($validated['order']) ? (int) $validated['order'] : ($competition->order ?? 0),
            'show_criteria' => $request->has('show_criteria') ? $request->boolean('show_criteria') : false,
        ...
        if (in_array($competition->code, ['MTQ', 'POP'])) {
            ...
            // Kuota Total Gabungan
            $totalQuota = $competition->quota;
            if ($request->filled($prefix.'_quota_total')) {
                $totalQuota = (int) $request->input($prefix.'_quota_total');
                AppSetting::set($prefix.'_quota_total', $totalQuota, 'pricing');
            } elseif ($request->filled($prefix.'_quota_pa') || $request->filled($prefix.'_quota_pi')) {
                $qPa = (int) $request->input($prefix.'_quota_pa', 0);
                $qPi = (int) $request->input($prefix.'_quota_pi', 0);
                $totalQuota = $qPa + $qPi;
            } elseif ($request->filled('quota')) {
                $totalQuota = (int) $request->input('quota');
            }
    ...
    /**
     * AJAX endpoint untuk Tab Rekap Asal Lembaga — dihitung on-demand saat tab diklik.
     * GET /admin/api/recap-institutions
     */
    public function apiRecapInstitutions()
    {
        $competitions = Competition::with([
            'category:id,name',
            'registrations' => function ($q) {
        ...
        foreach ($competitions as $comp) {
            $categoryName = $comp->category->name ?? 'Lomba';
            ...
            foreach ($comp->registrations as $reg) {
                ...
                $institutionGroups[$cleanKey]['total_registrations'] += 1;

                $isVerified = in_array($reg->status, ['verified', 'paid']);
        ...
        $institutionRecap = collect($institutionGroups)
            ->values()
            ->map(function ($item) {
        ...
        return response()->json([
            'institutions' => $institutionRecap,
            'total_institutions_count' => $institutionRecap->count(),
            'total_institution_students' => $institutionRecap->sum('total_students'),
