...
<head>
    ...
    <title>BUKTI PEMBUATAN AKUN - {{ $slip['nisn'] ?? ($user->nisn ?? 'RESMI') }} ({{ $slip['name'] ?? $user->name }})</title>
    ...
    <div class="no-print max-w-3xl mx-auto mb-4 px-4">
        <div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-sm font-black text-slate-900">Akun Berhasil Dibuat • {{ $appSettings['app_name'] ?? 'TALENTA 2026' }}</h1>
                    <p class="text-xs text-slate-500 font-mono">NISN: {{ $slip['nisn'] ?? ($user->nisn ?? '-') }} • {{ $slip['name'] ?? $user->name }}</p>
                </div>
            </div>
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('peserta.dashboard') }}" class="inline-flex items-center gap-1.5 px-5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow-md shadow-emerald-500/20 transition cursor-pointer">
