...
<head>
    ...
    <title>CETAK KOLEKTIF BUKTI PENDAFTARAN - ({{ $registrations->count() }} PESERTA)</title>
...
<body class="bg-slate-100 text-slate-900 font-sans antialiased min-h-screen py-4 sm:py-6">
    ...
    <div class="no-print max-w-4xl mx-auto mb-4 px-4 sticky top-4 z-50">
        <div class="bg-white/95 backdrop-blur-md p-4 rounded-2xl shadow-lg border border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-sm font-black text-slate-900">Cetak Kolektif Seluruh Bukti Pendaftaran</h1>
                    <p class="text-xs text-slate-500 font-medium">Total {{ $registrations->count() }} Formulir & Kartu Peserta Resmi Delegasi</p>
                </div>
            </div>
            <div class="flex items-center gap-2">
                ...
                <button onclick="window.print()" class="inline-flex items-center gap-2 px-5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow-md shadow-emerald-500/20 transition cursor-pointer">
                    ...
                    <span>Cetak Semua ({{ $registrations->count() }} Lembar)</span>
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-[210mm] mx-auto space-y-6">
        @foreach($registrations as $pageIndex => $registration)
            <div class="print-page bg-white pt-[1.5cm] px-[2cm] pb-[2cm] shadow-sm border border-slate-200 rounded-3xl flex flex-col justify-between">
                ...
                <div class="pt-2 border-t border-slate-200 flex items-center justify-between text-[9px] text-slate-400 font-mono">
                    ...
                    <span>Halaman {{ $pageIndex + 1 }} dari {{ $registrations->count() }} • Dicetak pada: {{ now()->format('d/m/Y H:i:s') }}</span>
                </div>
