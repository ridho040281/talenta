...
<head>
    ...
    <title>Buat Template WhatsApp Baru — {{ $appSettings['app_name'] ?? 'TALENTA' }} {{ $appSettings['institution_name'] ?? 'MTsN 1 Blitar' }}</title>
...
<body class="font-sans antialiased min-h-screen">
    ...
    <div style="position: sticky; top: 0; z-index: 100; background: rgba(9,13,23,0.97); border-bottom: 1px solid rgba(255,255,255,0.09); padding: 14px 32px; display: flex; align-items: center; justify-content: space-between; box-shadow: 0 4px 32px rgba(0,0,0,0.5);">
        <div class="flex items-center gap-3.5 min-w-0">
            ...
            <div class="min-w-0">
                <h3 class="text-sm sm:text-base font-black text-white truncate">Buat Template Pesan WhatsApp Kustom</h3>
                <p class="text-xs text-slate-400 hidden sm:block truncate">Tambahkan format pesan baru untuk keperluan siaran atau auto-trigger otomatis</p>
            </div>
        </div>
    ...
    <div class="max-w-4xl w-full mx-auto p-4 sm:p-6 lg:p-8 space-y-5">
        <form id="createTemplateForm" action="{{ route('admin.settings.whatsapp.blast.templates.store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div class="card-admin rounded-2xl p-5 sm:p-7 space-y-5">
                ...
                <div class="flex items-center justify-between pb-4 border-b border-white/[0.07]">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-2xl flex items-center justify-center shrink-0" style="background: rgba(16,185,129,0.15); border: 1px solid rgba(16,185,129,0.25);">
                            <i data-lucide="plus-circle" class="w-5 h-5 text-[#34d399]"></i>
                        </div>
                        <div>
                            <h4 class="text-sm font-black text-white">Informasi Template Baru</h4>
                            <p class="text-xs text-slate-400 mt-0.5">Format pesan dapat memuat variabel dinamis yang otomatis terisi data penerima</p>
                        </div>
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <input type="text" name="name" required value="{{ old('name') }}" placeholder="Contoh: Pengumuman Pembagian Sertifikat & Hadiah Juara" class="input-admin block w-full px-3.5 py-2.5 rounded-xl text-xs font-semibold">
                </div>

                <!-- Input: Deskripsi -->
                <div>
                    <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">Deskripsi Singkat (Opsional)</label>
                    <input type="text" name="description" value="{{ old('description') }}" placeholder="Contoh: Template siaran untuk memberitahukan waktu pengambilan piagam penghargaan..." class="input-admin block w-full px-3.5 py-2.5 rounded-xl text-xs">
                </div>
                ...
                <div>
                    <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">Isi Format Pesan WhatsApp *</label>
                    <textarea id="messageBox" name="message" rows="12" required placeholder="Tuliskan isi pesan WhatsApp di sini... Contoh:&#10;&#10;Assalamu'alaikum Wr. Wb.&#10;Yth. {nama_peserta} ({nama_sekolah}),&#10;Berikut informasi terkait cabang lomba *{cabang_lomba}*...&#10;&#10;Salam hangat,&#10;Panitia TALENTA 2026 MTsN 1 Blitar" class="input-admin block w-full px-4 py-3 rounded-xl text-xs font-mono leading-relaxed" style="tab-size: 4;">{{ old('message') }}</textarea>
                </div>
                ...
                <div class="pt-2">
                    <label class="flex items-center gap-3 p-3.5 rounded-xl cursor-pointer" style="background: rgba(16,185,129,0.07); border: 1px solid rgba(16,185,129,0.2);">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="w-4 h-4 rounded accent-emerald-500">
                        <div>
                            <span class="text-xs font-bold text-emerald-400">Aktifkan Template</span>
                            <p class="text-[11px] text-slate-400">Jika aktif, template ini siap digunakan untuk broadcast atau trigger pesan.</p>
                        </div>
                    </label>
                </div>
